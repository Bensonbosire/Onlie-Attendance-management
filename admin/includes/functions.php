...
function delete_std($conn,$table,$id){
 ...
 function teacher_entry($conn,$firstName,$lastName,$dob,$gender,$email,$phone,$degree,$salary,$address){
    ...
    function delete_teacher_record($conn,$table,$id){
    	try{
 ...
 /******************************/
	...
	try{
		...
		$query = "UPDATE  teacher_table SET first_name = ?, last_name = ?, dob = ?, gender = ?, email = ?, phone = ?, degree = ?, salary = ?, address = ? WHERE id = ?";
		$stmt = $conn->prepare($query);
	...
	try{
		...
		$query = "INSERT INTO subject_table SET Unitname=?, lecturername = ?, field = ? , semester =?";
...
function get_all_subject($conn,$table,$limit){
			...
			try {
				$query = "SELECT * FROM {$table} ORDER BY  Unitid LIMIT {$limit}";
					$stmt = $conn->prepare( $query );
					$stmt->execute();
					return $stmt->fetchAll();
